...
const getAccessCode = () => {
  ...
  logger.log('📊 전체 RUNTIME_CONFIG:', typeof window !== 'undefined' ? window.RUNTIME_CONFIG : 'undefined');

  // Railway 런타임 설정 확인
  if (typeof window !== 'undefined' && window.RUNTIME_CONFIG?.ACCESS_CODE) {
    logger.log('🔑 Railway 환경변수 사용:', window.RUNTIME_CONFIG.ACCESS_CODE);
    return window.RUNTIME_CONFIG.ACCESS_CODE;
  }

  // 개발 환경에서만 .env 파일 사용
  if (import.meta.env.MODE === 'development' && import.meta.env.VITE_ACCESS_CODE) {
    logger.log('🔧 개발 환경변수 사용:', import.meta.env.VITE_ACCESS_CODE);
